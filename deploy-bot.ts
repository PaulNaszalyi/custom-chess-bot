...
import { BotController } from './src/lichess/bot-controller';
import { LichessBotClient } from './src/lichess/lichess-bot-client';
import { BotDecisionEngine } from './src/engine/bot-decision-engine';
...
class BotDeployment {
    ...
    public async deployBot(): Promise<void> {
        try {
            ...
            const decisionEngine = new BotDecisionEngine(
                games,
                './data/patterns.json'
            );
            ...
            if (this.config.enableTournaments) {
                ...
                tournamentManager.startMonitoring();
            }
